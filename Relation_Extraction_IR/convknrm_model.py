...
class CONVKNRM(nn.Module):
    ...
    def __init__(self, args):
        ...
        matrix = np.zeros([len(self.index_to_word), 50], dtype=float)
        for i,word in enumerate(self.index_to_word):
            matrix[i] = self.wv[word]

        embed_mat = torch.cuda.FloatTensor(matrix)

        self.word_embeddings = nn.Embedding(num_embeddings=embed_mat.shape[0], embedding_dim=embed_mat.shape[1]).to(args.device)
    ...
    def forward(self, batch_queries, batch_docs):
        ...
        emb_query = self.word_embeddings(batch_queries)
        emb_desc = self.word_embeddings(batch_docs)
